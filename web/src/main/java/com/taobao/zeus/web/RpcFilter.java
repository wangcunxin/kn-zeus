	...
	public void doFilter(ServletRequest req, ServletResponse resp,
		...
		if (request.getRequestURI().endsWith(".rpc")) {
			...
			try {
				...
				// 		rpcRequest.getFlags());
                // log.info("#### RpcFilter #### start:" + rpcRequestMethod);
                // log.info("#### RpcFilter #### start:" + delegate);
                // log.info("#### RpcFilter #### start:" + rpcRequest.getParameters());
                // log.info("#### RpcFilter #### start:" + rpcRequest.getSerializationPolicy());
                // log.info("#### RpcFilter #### start:" + rpcRequest.getFlags());
				String responsePayload = RPC.invokeAndEncodeResponse(delegate,
						rpcRequestMethod, rpcRequest.getParameters(),
						rpcRequest.getSerializationPolicy(),
						rpcRequest.getFlags());
                // log.info("#### RpcFilter #### middle:" + rpcRequestMethod);
