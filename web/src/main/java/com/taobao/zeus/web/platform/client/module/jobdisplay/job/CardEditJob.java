...
import com.taobao.zeus.web.platform.client.module.jobmanager.NodeModel;
...
public class CardEditJob extends CenterTemplate implements
	...
	private TextButton save = new TextButton("保存", new SelectHandler() {
		...
		public void onSelect(final SelectEvent event) {
			...
			model.setHost(NodeModel.getIp(hostNodes.getValue()));
			model.setResponsibleMail(responsibleMailField.getValue());
			model.setMailReceivers(receivers.getValue());
			model.setMailCc(cc.getValue());
			model.setMailSubject(subject.getValue());
			model.setMailAttaches(attaches.getValue());
			model.setMailContent(content.getValue());
...
//	private TextField hostField;
	private ComboBox<String> hostNodes;
	private TextField responsibleMailField;
	...
	public CardEditJob(final JobPresenter presenter) {
		...
		centerContainer.add(getMailField(), new MarginData(3));
...
/*
	...
	public void refresh(final JobModel t) {
		...
		hostNodes.setValue(NodeModel.getName(t.getHost()));
		responsibleMailField.setValue(t.getResponsibleMail());
		receivers.setValue(t.getMailReceivers());
		cc.setValue(t.getMailCc());
		subject.setValue(t.getMailSubject());
		attaches.setValue(t.getMailAttaches());
		content.setValue(t.getMailContent());
...
//			hostField.setWidth(150);
			...
			responsibleMailField = new TextField();
			responsibleMailField.setWidth(150);
...
//								hostField.show();
			...
			ListStore<String> hostTypeStore = new ListStore<String>(
					new ModelKeyProvider<String>() {
						@Override
						public String getKey(final String item) {
							return item;
			...
			hostTypeStore.add(NodeModel.NONE_NODE.getName());
			hostTypeStore.add(NodeModel.MASTER.getName());
			hostTypeStore.add(NodeModel.SLAVE_01.getName());
			// hostTypeStore.add(NodeModel.SLAVE_02.getName());
			// hostTypeStore.add(NodeModel.SLAVE_03.getName());
			// hostTypeStore.add(NodeModel.SLAVE_04.getName());
			// hostTypeStore.add(NodeModel.SLAVE_03.getName());
			// hostTypeStore.add(NodeModel.SLAVE_04.getName());
			// hostTypeStore.add(NodeModel.SLAVE_05.getName());
			// hostTypeStore.add(NodeModel.SLAVE_06.getName());
			// hostTypeStore.add(NodeModel.SLAVE_07.getName());
			// hostTypeStore.add(NodeModel.SLAVE_08.getName());
			// hostTypeStore.add(NodeModel.SLAVE_09.getName());

			hostNodes = new ComboBox<String>(hostTypeStore,
					new LabelProvider<String>() {
						@Override
						public String getLabel(final String item) {
							return item;
			...
			hostNodes.setWidth(150);
			hostNodes.setTriggerAction(TriggerAction.ALL);
			hostNodes.setAllowBlank(true);
			hostNodes.setEditable(false);
			...
			rightContainer.add(new FieldLabel(hostNodes, "Host"), new VerticalLayoutData(1, -1));
			rightContainer.add(new FieldLabel(responsibleMailField, "责任人邮箱(多个责任人时以逗号分隔)"), new VerticalLayoutData(1, -1));
	...
	public FieldSet getMailField() {
		if (mailField == null) {
			mailField = new FieldSet();
			mailField.setCollapsible(true);
			mailField.setHeadingText("发送邮件信息");
			mailField.setWidth("96%");
			mailField.setResize(false);
			mailField.setHeight(320);

			HorizontalLayoutContainer layoutContainer=new HorizontalLayoutContainer();
			mailField.add(layoutContainer);

			VerticalLayoutContainer contentContainer = new VerticalLayoutContainer();
			contentContainer.setWidth(600);
			layoutContainer.add(contentContainer,new HorizontalLayoutData(-1,1));

			receivers = new TextField();
			receivers.setWidth(590);
			cc = new TextField();
			cc.setWidth(590);
			subject = new TextField();
			subject.setWidth(590);
			attaches = new TextField();
			attaches.setWidth(590);
			contentContainer.add(new FieldLabel(receivers, "收件人"));
			contentContainer.add(new FieldLabel(cc, "抄送"));
			contentContainer.add(new FieldLabel(subject, "主题"));
			contentContainer.add(new FieldLabel(attaches, "附件"));
