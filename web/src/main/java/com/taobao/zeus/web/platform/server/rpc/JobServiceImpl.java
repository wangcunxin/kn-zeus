...
//				.getUser().getUid(), jobId));
		jobModel.setAdmin(Boolean.TRUE);
		...
		jobModel.setResponsibleMail(jobBean.getJobDescriptor().getResponsibleMail());
		jobModel.setTaskId(jobBean.getJobDescriptor().getTaskId());
		jobModel.setMailReceivers(jobBean.getJobDescriptor().getMailReceivers());
		jobModel.setMailCc(jobBean.getJobDescriptor().getMailCc());
		jobModel.setMailSubject(jobBean.getJobDescriptor().getMailSubject());
		jobModel.setMailAttaches(jobBean.getJobDescriptor().getMailAttaches());
		jobModel.setMailContent(jobBean.getJobDescriptor().getMailContent());
		return jobModel;
	}
	...
	public JobModel updateJob(JobModel jobModel) throws GwtException {
		...
		jd.setResponsibleMail(jobModel.getResponsibleMail());
		jd.setMailReceivers(jobModel.getMailReceivers());
		jd.setTaskId(jobModel.getTaskId());
		jd.setMailCc(jobModel.getMailCc());
		jd.setMailSubject(jobModel.getMailSubject());
		jd.setMailAttaches(jobModel.getMailAttaches());
		jd.setMailContent(jobModel.getMailContent());
		try {
	...
	public void run(String jobId, int type) throws GwtException {
	    ...
	    // log.info("#### JobServiceImpl run #### jobId:" + jobId + ", type:" + type);
