				...
				if(zeusSchedule!=null){
					...
					if(context!=null){
						...
						if("workers".equals(op)){
							...
							resp.getWriter().println("worker count: " + workers.size()+":");
							SimpleDateFormat format=new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
							for(Channel channel:workers.keySet()){
								resp.getWriter().println("\t channel: "+channel.toString() + " id: " + channel.getId());
								...
								if (null == heart || null == heart.timestamp) continue;
								resp.getWriter().println("\t\t last heartbeat:"+ format.format(heart.timestamp));
								resp.getWriter().println("\t\t mem use rate:"+heart.memRate);
								resp.getWriter().println("\t\t cpu load per core:"+heart.cpuLoadPerCore);
								resp.getWriter().println("\t\t net rate:"+heart.netRate);
								resp.getWriter().println("\t\t heart host:"+heart.host);
