...
public class GroupServiceImpl implements GroupService{
	...
	@Override
	public GroupInherit getParentGroups(String groupId) throws GwtException {
		GroupBean groupBean = permissionGroupManager.getUpstreamGroupBean(groupId);
		log.info("##############group%%%%%%%%%%%%%%", groupBean.getGroupDescriptor().getId() + "#"
				+ groupBean.getParentGroupBean().getGroupDescriptor().getId() + "#"
				+ groupBean.getParentGroupBean().getParentGroupBean().getGroupDescriptor().getId() + "#"
				+ groupBean.getParentGroupBean().getParentGroupBean().getParentGroupBean().getGroupDescriptor().getId());
		GroupInherit group = new GroupInherit();
		copyData(groupBean, group);
		return group;
	}

	public void copyData(GroupBean groupBean, GroupInherit group) {
		if (groupBean.getParentGroupBean() == null) {
			group.setGroupId(groupBean.getGroupDescriptor().getId());
			group.setParentGroup(null);
		} else {
			group.setGroupId(groupBean.getGroupDescriptor().getId());
			GroupInherit parent = new GroupInherit();
			group.setParentGroup(parent);
			copyData(groupBean.getParentGroupBean(), parent);
		}
	}
