...
import com.taobao.zeus.web.platform.client.module.jobmanager.GroupInherit;
...
public class CardInfo extends CenterTemplate implements Refreshable<JobModel>{
	...
	private FieldLabel responsibleMail;
	...
	private FieldLabel receviers;
	private FieldLabel cc;
	private FieldLabel subject;
	private FieldLabel attaches;
	private TextArea content;
	
	private JobPresenter presenter;
	
	public CardInfo(final JobPresenter presenter){
...
////		addButton(toDoc);
	...
	private void display(final JobModel model){
		...
		((Label)host.getWidget()).setText(NodeModel.getName(model.getHost()));
		((Label)responsibleMail.getWidget()).setText(model.getResponsibleMail());

		// mail info display add tdj
		((Label)receviers.getWidget()).setText(model.getMailReceivers());
		((Label)cc.getWidget()).setText(model.getMailCc());
		((Label)subject.getWidget()).setText(model.getMailSubject());
		((Label)attaches.getWidget()).setText(model.getMailAttaches());
		content.setText(model.getMailContent());
		...
		RPCS.getUserService().getUser(new AsyncCallback<ZUser>() {
			@Override
			public void onSuccess(ZUser result) {
				if (result.getUid().equals("data_analysis")) {
					RPCS.getGroupService().getParentGroups(model.getGroupId(), new AsyncCallback<GroupInherit>() {
						@Override
						public void onFailure(Throwable throwable) {
							throwable.printStackTrace();
						}
						...
						@Override
						public void onSuccess(GroupInherit result) {
							if (findAnalysisParent(result)) {
								history.enable();
								depGraph.enable();
								editGroup.enable();
								execute.enable();
								...
								deleteGroup.enable();
								configAdmin.enable();
							} else if (findPlatformParent(result)) {
								history.enable();
								depGraph.enable();
								editGroup.disable();
								execute.enable();
						...
						public boolean findAnalysisParent(GroupInherit group) {
							final String ANALYSIS_GROUP_ID = "121";
//							final String ANALYSIS_GROUP_ID = "5";

							if(null == group.getParentGroup()) {
								return false;
							} else if (group.getGroupId().equals(ANALYSIS_GROUP_ID)) {
								return true;
							} else {
								return findAnalysisParent(group.getParentGroup());
							}
						}

						public boolean findPlatformParent(GroupInherit group) {
							final String PLATFORM_GROUP_ID = "73";
//							final String PLATFORM_GROUP_ID = "6";

							if(null == group.getParentGroup()) {
								return false;
							} else if (group.getGroupId().equals(PLATFORM_GROUP_ID)) {
								return true;
							} else {
								return findPlatformParent(group.getParentGroup());
							}
						}
	...
	public FieldSet getBaseFieldSet() {
		if(baseFieldSet==null){
			...
			responsibleMail=new FieldLabel(getLabel(),"责任人邮箱(多个责任人时以逗号分隔)");
			...
			rightContainer.add(responsibleMail,new VerticalLayoutContainer.VerticalLayoutData(1, -1));
	...
	public FieldSet getMailField() {
		if(mailField==null){
			...
			receviers=new FieldLabel(getLabel(),"收件人");
			cc=new FieldLabel(getLabel(),"抄送");
			subject=new FieldLabel(getLabel(),"主题");
			attaches=new FieldLabel(getLabel(),"附件");
			content=new TextArea();
			content.setWidth(700);
			content.setHeight(180);
			content.setEnabled(false);
			contentContainer.add(receviers, new VerticalLayoutContainer.VerticalLayoutData(1, -1));
			contentContainer.add(cc, new VerticalLayoutContainer.VerticalLayoutData(1, -1));
			contentContainer.add(subject, new VerticalLayoutContainer.VerticalLayoutData(1, -1));
			contentContainer.add(attaches, new VerticalLayoutContainer.VerticalLayoutData(1, -1));
