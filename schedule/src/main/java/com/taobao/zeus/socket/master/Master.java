...
import static com.taobao.zeus.model.JobStatus.TimeoutFlag.IS_TIMEOUT;
...
public class Master {
	...
	public Master(final MasterContext context) {
		...
		context.getSchedulePool().scheduleAtFixedRate(new Runnable() {
			...
			public void run() {
				try {
					List<Controller> controllers = context.getDispatcher().getControllers();
					if (null != controllers && 0 != controllers.size()) {
						for(Controller c : controllers) {
							if(c instanceof JobController){
								JobController jc = (JobController)c;
								JobDescriptor jd = context.getGroupManager().getJobDescriptor(jc.getJobId()).getX();
								if (jd.getAuto()) {
									String timeoutPoint = jd.getProperties().get("job.timeout.point");
									if (timeoutPoint == null || timeoutPoint.trim().isEmpty()) {
										continue;
									}
									...
									String ymdStr = sdf.format(now);
									String timeoutCompleteStr = ymdStr + " " + timeoutPoint;
									Date timeoutDate = null;
									try {
										timeoutDate = sdf2.parse(timeoutCompleteStr);
									} catch (Exception e) {
										log.error("timeout point wrong format: " + timeoutPoint, e);
										continue;
									}
									if (null == timeoutDate) continue;

									if (timeoutDate.compareTo(now) <= 0) {
										boolean isAlarm = true;

										List<JobTimeOutFlag> timeOutFlags = context.getJobTimeOutFlagManager().getJobTimeOutFlagList(jc.getJobId(), ymdStr);
										if (null != timeOutFlags && timeOutFlags.size() > 0) {
											isAlarm = false;
										}

										String startTimeStr = ymdStr + " " + "00:00:00";
										Date startDate = sdf2.parse(startTimeStr);
										List<JobHistory> jobHistoryList = context.getJobHistoryManager().getTimeIntervalHistoryList(jc.getJobId(), startDate, timeoutDate);

										if (null != jobHistoryList && jobHistoryList.size() > 0) {
											isAlarm = false;
										}

										if(isAlarm) {
											ScheduleInfoLog.info("job id put to time out queue: " + jc.getJobId());

											String dingdingGroup = jd.getProperties().get("dingding.group");
											if (null != dingdingGroup && !"".equals(dingdingGroup.trim())) {
												...
												for(String ddGroup : ddGroupArr) {
													...
													jobResult.setIsTimeOut(1);
													jobResult.setTimeoutRemark("任务在" + timeoutCompleteStr + "未成功运行完成");
													TimeOutJobQueue.put(jobResult);
												}
											} else {
												JobResult jobResult = new JobResult();
												jobResult.setJob(jd);
											...
											JobTimeOutFlag jobTimeOutFlag = new JobTimeOutFlag();
											jobTimeOutFlag.setJobId(Long.valueOf(jc.getJobId()));
											jobTimeOutFlag.setTimeoutDate(ymdStr);
											context.getJobTimeOutFlagManager().addTimeoutJobFlag(jobTimeOutFlag);
										}
									}
								}
							}
					...
					ScheduleInfoLog.error("timeout time point alarm fails " , e);
					log.error("timeout time point alarm fails ", e);
				}
			}
		...
		context.getSchedulePool().scheduleAtFixedRate(new Runnable() {
			public void run() {
				String jobId = "";
				try {
					JobResult jobResult = TimeOutJobQueue.take();
...
//			ScheduleInfoLog.info("master get cpuLoadPerCore: " + heart.cpuLoadPerCore);
//			ScheduleInfoLog.info("master get rate: " + heart.netRate);
			if (heart != null && heart.memRate != null && heart.memRate < 0.8
					&& heart.cpuLoadPerCore != null && heart.cpuLoadPerCore < Environment.getMaxCpuLoadPerCore()
					&& heart.netRate != null && heart.netRate < Environment.getMaxNetRate()) {
				if (selectWorker == null) {
					selectWorker = worker;
					netRate = heart.netRate;
				} else if (netRate > heart.netRate) {
					selectWorker = worker;
					netRate = heart.netRate;
				}
			}
	...
	private MasterWorkerHolder getRunableWorker(String host) {
		MasterWorkerHolder selectWorker = null;
		ScheduleInfoLog.info("********************host:" + host);
		if (host != null && !"".equals(host)) {
			for (MasterWorkerHolder worker : context.getWorkers().values()) {
				HeartBeatInfo heart = worker.getHeart();
//				ScheduleInfoLog.info("#############host:" + host + "heart host:"
//						+ heart.host);
//				ScheduleInfoLog.info("master get cpuLoadPerCore: " + heart.cpuLoadPerCore);
//				ScheduleInfoLog.info("master get rate: " + heart.netRate);
				if (heart != null && heart.memRate != null && heart.memRate < 0.8
	...
	private void scan() {
		...
		if (!context.getManualQueue().isEmpty()) {
			...
			MasterWorkerHolder selectWorker = getRunableWorker(e.getHost());
			// tdj 2017-05-10
			ScheduleInfoLog.info("scan queue poll: " + e.getJobID());
			if (selectWorker == null) {
				ScheduleInfoLog.info("scan queue re input job to queue: " + e.getJobID());
				context.getManualQueue().offer(e);
			} else {
				ScheduleInfoLog.info("scan queue and begin to call manuual job " + e.getJobID());
	...
	private void checkManualTimeOver(MasterWorkerHolder w) {
		for (Map.Entry<String, Boolean> entry : w.getManualRunnings()
			...
			if (IS_TIMEOUT.getTimeoutFlag().intValue() == his.getTimeoutFlag()) {
				continue;
			}
...
//				}
				// 超时发钉钉
				try {
					ScheduleInfoLog.info("job history id put to time out queue: " + his.getId());
					...
					if (null != dingdingGroup && !"".equals(dingdingGroup.trim())) {
						...
						for(String ddGroup : ddGroupArr) {
							...
							jobResult.setWebhookToken(webhookToken);
							jobResult.setIsTimeOut(1);
							jobResult.setTimeoutRemark("任务运行超过" + maxTimeString + "分钟");
							TimeOutJobQueue.put(jobResult);
						}
					} else {
						JobResult jobResult = new JobResult();
					...
					// 更新超时标记
					context.getJobHistoryManager().updateTimeoutFlag(IS_TIMEOUT.getTimeoutFlag(), his.getId());
				} catch (Exception e) {
					ScheduleInfoLog.error("failed to send timeout dingding message for job id:" + jd.getId(), e);
				}
			}
		}
	}
