...
public class WorkerHeartBeat {
	...
	public ChannelFuture execute(WorkerContext context) {
		...
		ScheduleInfoLog.info("worker cpuLoadPerCore: " + jobContext.getData("cpuLoadPerCore"));
		ScheduleInfoLog.info("worker net rate: " + context.getCurrentNetRate());
		...
		HeartBeatMessage hbm = HeartBeatMessage.newBuilder()
				...
				.setTimestamp(new Date().getTime()).setHost(host)
				.setCpuLoadPerCore((Float)jobContext.getData("cpuLoadPerCore"))
				.setNetRate(context.getCurrentNetRate()).build();
