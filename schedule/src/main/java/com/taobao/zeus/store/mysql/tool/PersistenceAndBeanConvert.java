...
public class PersistenceAndBeanConvert {
	...
	public static Tuple<JobDescriptor, JobStatus> convert(JobPersistence persist) {
		...
		jd.setResponsibleMail(persist.getResponsible_mail());
		jd.setTaskId(persist.getTaskId());
		// mail info
		jd.setMailReceivers(persist.getMail_receivers());
		jd.setMailCc(persist.getMail_cc());
		jd.setMailSubject(persist.getMail_subject());
		jd.setMailAttaches(persist.getMail_attaches());
		jd.setMailContent(persist.getMail_content());
	...
	public static JobHistory convert(JobHistoryPersistence persist) {
		...
		history.setTimeoutFlag(persist.getTimeoutFlag());
		return history;
	}
	...
	public static JobHistoryPersistence convert(JobHistory history) {
		...
		if (history.getTimeoutFlag() != null) {
			persist.setTimeoutFlag(history.getTimeoutFlag());
		}

		return persist;
	}

	public static JobTimeOutFlag convert(JobTimeOutFlagPersistence persist) {
		if (persist == null) {
			return null;
		}
		JobTimeOutFlag timeOutFlagJob = new JobTimeOutFlag();
		timeOutFlagJob.setId(persist.getId());
		timeOutFlagJob.setJobId(persist.getJobId());
		timeOutFlagJob.setTimeoutDate(persist.getTimeoutDate());
		return timeOutFlagJob;
	}

	public static JobTimeOutFlagPersistence convert(JobTimeOutFlag timeOutFlagJob) {
		if (timeOutFlagJob == null) {
			return null;
		}
		JobTimeOutFlagPersistence persist = new JobTimeOutFlagPersistence();
		persist.setId(timeOutFlagJob.getId() == null ? null : timeOutFlagJob.getId());
		persist.setTimeoutDate(timeOutFlagJob.getTimeoutDate());
		persist.setJobId(timeOutFlagJob.getJobId());
		return persist;
	}
