...
public class MysqlJobHistoryManager extends HibernateDaoSupport implements JobHistoryManager{
	...
	@Override
	public void updateTimeoutFlag(final Integer timeoutFlag, final String id) {
		getHibernateTemplate().execute(new HibernateCallback() {

			@Override
			public Object doInHibernate(Session session) throws HibernateException,
					SQLException {
				Query query=session.createQuery("update com.taobao.zeus.store.mysql.persistence.JobHistoryPersistence set timeout_flag=? where id=?");
				query.setParameter(0, timeoutFlag);
				query.setParameter(1, Long.valueOf(id));
				query.executeUpdate();
				return null;
			}
	...
	@Override
	public List<JobHistory> getTimeIntervalHistoryList(final String jobId, final Date startTime, final Date endTime) {
		return (List<JobHistory>) getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session) throws HibernateException,
					SQLException {
				String sql="select id,job_id,start_time,end_time,execute_host,status,trigger_type,illustrate,operator,properties from zeus_job_history where job_id = ? and end_time >= ? and end_time <= ? and status = 'success' order by end_time desc";
				SQLQuery query=session.createSQLQuery(sql);
