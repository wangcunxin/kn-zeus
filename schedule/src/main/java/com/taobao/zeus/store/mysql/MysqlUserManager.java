...
public class MysqlUserManager extends HibernateDaoSupport implements UserManager{
	...
	public ZeusUser checkUser(final String uid, final String password) {

		if (null == uid || null == password) return null;
		...
		List<ZeusUser> list=(List<ZeusUser>) getHibernateTemplate().execute(new HibernateCallback() {
			...
			public Object doInHibernate(Session session) throws HibernateException,
					SQLException {
				Query query=session.createQuery("from com.taobao.zeus.store.mysql.persistence.ZeusUser where uid=? and password = MD5(?)");
